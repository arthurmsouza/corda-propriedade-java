...
public class PropriedadeContract implements Contract {
    ...
    public void verify(LedgerTransaction tx) throws IllegalArgumentException {
        ...
        else if (commandType instanceof  PropertyCommand.Transferir) verificarTrasferir(tx,command);
        else if (commandType instanceof PropertyCommand.AprovacaoBanco) verificarAprocaoBanco(tx,command);
    }

    private void verificarAprocaoBanco(LedgerTransaction tx, CommandWithParties<PropertyCommand> command) {
        requireThat(require -> {
            return null;
        });
    }

    private void verificarTrasferir(LedgerTransaction tx, CommandWithParties<PropertyCommand> command) {
        requireThat(require -> {
            require.using("A propriedade  deve ter apenas um input",tx.getInputs()
                    .size()==1);
            require.using("A propriedade  deve ter apenas um output",tx.getOutputs()
                    .size()==1);
    ...
    private void verificaCriar(LedgerTransaction tx, CommandWithParties<PropertyCommand> command) {
        requireThat(require -> {
            require.using("A propriedade nao deve ter nenhum input",tx.getInputs()
                    .isEmpty());
            require.using("A propriedade deve ter apenas um output ",tx.getOutputs()
                    .size() == 1);
            final PropriedadeState out = tx.outputsOfType(PropriedadeState.class).get(0);
